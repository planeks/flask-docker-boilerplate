...
Download the last version of the boiler plate from the repository: https://github.com/planeks/flask-docker-boilerplate/
...
```shell
$ docker compose -f docker-compose.dev.yml exec flask bash
```

Or, you can run the temporary container:

```shell
$ docker compose -f docker-compose.dev.yml run --rm flask bash
```
...
2. SSH into the server and check:
   ```bash
   cd ~/projects/flask-docker-boilerplate
