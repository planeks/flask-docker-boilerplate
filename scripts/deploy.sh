...
if [ "$MISSING_SECRETS" = true ]; then
    ...
    if is_placeholder "${SECRET_KEY:-}"; then
        ...
        log_info "   python -c 'import secrets; print(secrets.token_urlsafe(50))'"
        echo ""
    fi
    if is_placeholder "${POSTGRES_PASSWORD:-}"; then
...
# Check SITE_DOMAIN (for production)
if [ "$COMPOSE_FILE" = "compose.prod.yml" ]; then
    if [[ "${SITE_DOMAIN:-}" == "example.com" ]] || is_placeholder "${SITE_DOMAIN:-}"; then
        log_warning "SITE_DOMAIN is not properly configured!"
        log_warning "Please update SITE_DOMAIN in .env file with your actual domain."
    fi
fi
...
# Wait additional time for Flask to start and run migrations
log_info "Waiting for Flask to complete startup..."
sleep 15
...
HEALTH_CHECK_OUTPUT=$(docker compose -f "$COMPOSE_FILE" exec -T flask bash -c "cd /opt/project/src && poetry run python -c 'from app import server; print(\"Flask app OK\")'") || {
    log_error "Flask health check failed!"
    log_error "Health check output:"
    echo "$HEALTH_CHECK_OUTPUT"
    log_error "Flask container logs:"
    docker compose -f "$COMPOSE_FILE" logs --tail=50 flask
