...
Create the `appuser` group with GID 1024. This is used for non-root volume permissions.

```bash
$ sudo addgroup --gid 1024 appuser
```
...
Set `SITE_DOMAIN` to your actual domain. `COMPOSE_PROJECT_NAME` is used as a prefix for container images and can match your dev configuration.
...
```bash
ssh -i <your-key> <user>@<server_ip> "bash ~/projects/flask_app/scripts/health-check.sh"
```
...
#### Flask container fails to start

Symptoms:
- Deployment script reports: `[ERROR] Some services failed to start: flask_app-flask-run-*`
- Flask container exits immediately after starting
...
2. **Placeholder values in .env**
   ...
   - `SITE_DOMAIN` - your actual domain (not `example.com`)
   ...
   ./scripts/deploy.sh compose.dev.yml main ~/projects/flask_app
   ```
...
2. SSH into the server and check container logs:
   ```bash
   cd ~/projects/flask_app
   docker compose -f compose.prod.yml logs flask
