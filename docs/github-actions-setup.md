...
grep -E 'SECRET_KEY|POSTGRES_PASSWORD|SITE_DOMAIN' .env
```
...
```bash
ssh -i <your-key> ubuntu@54.123.45.67 "ls -la ~/projects/flask_app/.env"
```

2. Copy from template:
```bash
ssh -i <your-key> ubuntu@54.123.45.67 "cp ~/projects/flask_app/.env.template ~/projects/flask_app/.env"
```
