...
```bash
docker compose -f compose.dev.yml exec flask ruff check .          # lint
docker compose -f compose.dev.yml exec flask ruff check --fix .    # lint and auto-fix
docker compose -f compose.dev.yml exec flask ruff format .         # format
docker compose -f compose.dev.yml exec flask ruff format --check . # check formatting without changes
```
...
2. **test-backend** -- Flask tests in Docker (runs after lint-backend passes)
