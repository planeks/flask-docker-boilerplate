...
2. **Configure local environment**
   ...
   - Edit `ansible/.env.ansible` and set required variables:
     ...
     - `PROJECT_NAME` - project name (default: `flask_app`), used for the app directory name
...
1. **Add GitHub secrets** (required for deployments)
   ...
   >
   > Project path defaults to `~/projects/flask_app`. If you changed `PROJECT_NAME`
...
2. **Verify `.env` on server** - provisioning creates it from the template with generated secrets. Check that values are set:
   ```bash
   ssh -i <your-key> <user>@<server_ip> "grep -E 'SECRET_KEY|POSTGRES_PASSWORD|SITE_DOMAIN' ~/projects/<project_name>/.env"
   ```
